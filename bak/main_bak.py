...
cell_f = MeshFunction('size_t', mesh, 2)
for cell in cells(mesh):
    if cell.midpoint().x() < 2.5e-4:
        ...
        cell_f[cell] = 2
mesh1 = SubMesh(mesh, cell_f, 0)
mesh2 = SubMesh(mesh, cell_f, 1)
mesh3 = SubMesh(mesh, cell_f, 2)
# plot(mesh1)
# Define function space for velocity
W = VectorFunctionSpace(mesh, 'P', 2)
# V = FunctionSpace(mesh, 'P', 1)
# V1 = FiniteElement('P', triangle, 1)
# V2 = FunctionSpace(mesh, 'P', 1)
# V3 = FunctionSpace(mesh, 'P', 1)
...
bc_electric_potential = DirichletBC(V.sub(0), u_electric_potential, boundary_electric_potential)
...
#         value[0] = i_a(x[0], x[1], phis1F, philF)
# class ICExpression(UserExpression):
#     def eval(self, value, x):
#         value[0] = i_c(x[0], x[1], phis3F, philF)
# g_L = IAExpression()
# g_R = ICExpression()
# if loop==0:
boundary_conditions = {0: {'Dirichlet': u_electric_gnd},
                       ...
                       4: {'Neumann': 0}}
# else:
#     boundary_conditions[1]['Neumann']=g_L
#     boundary_conditions[2]['Neumann'] = g_R
...
for i in boundary_conditions:
    if 'Neumann' in boundary_conditions[i]:
        if boundary_conditions[i]['Neumann'] != 0:
            ...
            if i == 1:
                integrals_N1.append(-g * v1 * ds(i))
                integrals_N2.append(g * v2 * ds(i))
            elif i == 2:
                integrals_N2.append(g * v2 * ds(i))
                integrals_N3.append(-g * v3 * ds(i))
...
#             integrals_IN.append(g * v1 * ds(i))
#             integrals_IN.append(-g * v2 * ds(i))


f = Constant(0.0)
# a = 10 * dot(grad(u), grad(v1)) * dx(0)+ dot(grad(u), grad(v1)) * dx(2)

a1 = kappa_s * dot(grad(phis1), grad(v1)) * dx(0)
L1 = f * v1 * dx(0) - sum(integrals_N1)
# L1 = - sum(integrals_N) - sum(integrals_IN)
# L1 = f * v1 * dx - g_L * v1 * ds1(1)
# L = f * v1 * dx - 2800 * v1 * ds(1) + 2800 * v1 * ds(2)
a2 = kappa_m * dot(grad(phil), grad(v2)) * dx(1)
L2 = f * v2 * dx(1) - sum(integrals_N2)
a3 = kappa_s * dot(grad(phis3), grad(v3)) * dx(2)
L3 = f * v3 * dx(2) - sum(integrals_N3)
...
solve(a1 == L1, phis1F, bc1)
# # solve(a2 == L2, philF, bc2)
# # solve(a3 == L3, phis3F, bc3)

# F = dot(grad(phis1), grad(v1))*dx - f * v1 * dx
...
# screenshot(filename='main.png')
# exportWindow("main/phis1.pvd")
